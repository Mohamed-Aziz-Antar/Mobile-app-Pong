...
class TimeAttackMode extends StatefulWidget {
  const TimeAttackMode({Key? key}) : super(key: key);

  @override
  State<TimeAttackMode> createState() => _TimeAttackModeState();
}

class _TimeAttackModeState extends State<TimeAttackMode> with WidgetsBindingObserver {
  ...
  final double _accelerometerSensitivity = 8.0; // Sensitivity for accelerometer input
  ...
  int _timeLimit = 1; // Default time limit in minutes
  int _remainingTime = 60; // Remaining time in seconds
  Timer? _gameTimer; // Timer for the game countdown
  ...
  void _checkGameOver() {
    if (playerScore >= _pointsToWin || aiScore >= _pointsToWin || _remainingTime <= 0) {
      setState(() {
        _gameOver = true;
        _isGamePaused = true;
        _gameTimer?.cancel();
  ...
  void _startTimer() {
    _remainingTime = _timeLimit * 60; // Convert minutes to seconds
    _gameTimer?.cancel();
  ...
  void _updateGameSettings(double speed, String difficulty, int pointsToWin, int timeLimit) {
    setState(() {
            ...
            content: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  const Text("Time Limit (Minutes)", style: TextStyle(color: Colors.white70, fontSize: 14)),
                  const SizedBox(height: 8),
                  Container(
                    height: 40,
                    decoration: BoxDecoration(
                    ...
                    child: Row(
                      children: [1, 2, 5].map((option) {
                        bool isSelected = timeLimit == option;
                        return Expanded(
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      onSettingsChanged(gameSpeed, selectedPaddle, pointsToWin, timeLimit);
                      Navigator.pop(context);
                    },
